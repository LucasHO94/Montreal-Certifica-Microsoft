import { questions } from '../src/data/questions/ms700/pt.js';

function audit() {
  console.log("=== INICIANDO AUDITORIA SIMULADO MS-700 ===");
  ...
  questions.forEach((q, idx) => {
    ...
    if (!q.id) {
      ...
      const expectedId = `ms700_q${idx + 1}`;
      if (q.id !== expectedId) {
        console.error(`ERRO: Questão no índice ${idx} tem ID incorreto. Esperado: ${expectedId}, obtido: ${q.id}`);
        errors++;
      }
    }
    ...
    // Check correctAnswer
    const expectedCorrect = idx % 4;
    if (q.correctAnswer === undefined || q.correctAnswer < 0 || q.correctAnswer > 3) {
      console.error(`ERRO [${q.id || idx}]: 'correctAnswer' inválido ou ausente: ${q.correctAnswer}`);
      errors++;
    } else if (q.correctAnswer !== expectedCorrect) {
      console.error(`ERRO [${q.id || idx}]: 'correctAnswer' está incorreto no balanceamento cíclico. Esperado: ${expectedCorrect}, obtido: ${q.correctAnswer}`);
      errors++;
    } else {
