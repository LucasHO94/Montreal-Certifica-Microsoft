    ...
    switch (event.type) {
      case 'checkout.session.completed': {
        ...
        await supabase
          .from('profiles')
          .update({ 
            az900_is_premium: true, 
            az900_stripe_customer_id: customerId,
            az900_subscription_id: subscriptionId 
          })
          .eq('id', userId)
      ...
      case 'customer.subscription.deleted': {
        ...
        await supabase
          .from('profiles')
          .update({ az900_is_premium: false })
          .eq('az900_subscription_id', subscription.id)
        break
      }
    }
