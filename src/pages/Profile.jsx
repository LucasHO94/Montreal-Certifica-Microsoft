  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-slate-50 flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="font-bold text-slate-400">{t('loading_data')}</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 font-sans text-slate-800 pb-20">
      ...
      <nav className="bg-white border-b border-slate-200 sticky top-0 z-40 shadow-sm">
        <div className="max-w-5xl mx-auto px-4 h-16 flex justify-between items-center">
          ...
          <div className="font-black text-xl text-slate-800 tracking-tight">{t('profile_title')}</div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-1 space-y-6">
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 text-center relative overflow-hidden">
                ...
                <h2 className="text-xl font-black text-slate-800 mb-1">{profile?.full_name || t('beginner')}</h2>
                ...
                <div className="flex justify-center gap-2 mb-8">
                    {profile?.montreal_is_premium ? (
                        ...
                        <span className="bg-slate-100 text-slate-500 px-3 py-1 rounded-full text-[10px] font-black flex items-center gap-1">
                            <Shield size={10} /> {t('free_plan').toUpperCase()}
                        </span>
                    )}
                </div>
                ...
                <div className="space-y-4 text-left border-t border-slate-100 pt-6">
                    ...
                    <div className="flex items-center gap-3 text-sm font-bold text-slate-500">
                        <Calendar size={16} /> {t('member_since')} {new Date(profile?.created_at).toLocaleDateString()}
                    </div>
                </div>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-lg p-8">
                <h3 className="font-black text-slate-800 mb-6 flex items-center gap-2 text-sm">
                    <Award size={18} className="text-blue-600" /> {t('next_achievement')}
                </h3>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 md:p-10">
                <h3 className="font-black text-2xl text-slate-800 mb-8 flex items-center gap-3">
                    <Edit2 size={24} className="text-blue-600" /> {t('profile_settings')}
                </h3>
                
                <form onSubmit={handleSave} className="space-y-6">
                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">{t('unique_nickname')}</label>
                        ...
                        <p className="text-[10px] text-slate-400 mt-2 px-1 font-bold italic">{t('nickname_hint')}</p>
                    </div>

                    <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 px-1">{t('full_name_label')}</label>
                        <input 
                            type="text" 
                    ...
                    >
                        {saving ? t('saving') : <><Save size={18} /> {t('save_changes')}</>}
                    </button>
                </form>
            </div>
            ...
            <div className="bg-white rounded-[2.5rem] border border-slate-200 shadow-xl p-8 md:p-10">
                <h3 className="font-black text-2xl text-slate-800 mb-8 flex items-center gap-3">
                    <PieChart size={24} className="text-blue-600" /> {t('stats_numbers')}
                </h3>
                
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                    <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                        <p className="text-[10px] font-black text-slate-400 uppercase mb-2">{t('questions')}</p>
                        <p className="text-4xl font-black text-slate-800 tracking-tighter">{stats.totalSimulados}</p>
                    </div>
                    <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                        <p className="text-[10px] font-black text-slate-400 uppercase mb-2">{t('best_note')}</p>
                    ...
                    <div className="p-6 bg-slate-100 rounded-3xl border border-slate-200">
                        <p className="text-[10px] font-black text-slate-500 uppercase mb-2">{t('avg_general')}</p>
                        <p className="text-4xl font-black text-slate-900 tracking-tighter">{stats.avgScore}%</p>
                    </div>
                    <div className="p-6 bg-slate-50 rounded-3xl border border-slate-100">
                        <p className="text-[10px] font-black text-slate-400 uppercase mb-2">{t('corrects')}</p>
                ...
                <div className="mt-8 p-6 bg-blue-50/50 rounded-3xl border border-blue-100 flex items-center gap-6">
                    ...
                    <div>
                        <h4 className="font-black text-blue-900 text-lg">{t('acc_precision')}</h4>
                        <p className="text-sm font-bold text-blue-700/60 mb-2">{t('acc_precision_desc').replace('{total}', stats.totalQuestions)}</p>
